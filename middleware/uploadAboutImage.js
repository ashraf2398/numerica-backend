...
const fileFilter = (req, file, cb) => {
  console.log('Processing about image file:', file.fieldname, file.originalname);
...
const uploadAboutImage = (req, res, next) => {
  upload.any()(req, res, (err) => {
    if (err) {
      return handleAboutUploadError(err, req, res, next);
    }
    ...
    if (req.files && req.files.length > 0) {
      ...
      if (imageFile) {
        req.file = imageFile;
        console.log('About image file uploaded:', req.file.filename);
      }
    }
    
    console.log('About form data received:', req.body);
    console.log('About files received:', req.files ? req.files.map(f => f.fieldname) : 'No files');
...
const handleAboutUploadError = (err, req, res, next) => {
  ...
  if (err instanceof multer.MulterError) {
    ...
    if (err.code === 'LIMIT_UNEXPECTED_FILE') {
      return res.status(400).json({ 
        error: `Unexpected file field "${err.field}". Only "image" field is allowed for about image uploads.`,
